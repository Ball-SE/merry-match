...
interface OtherSwipeListProps {
  ...
  callMerryAPI?: (swiped_id: string | number, action: "like" | "pass") => void;
}

export default function OtherSwipeList({ 
  otherSwipeList, 
  handleProfile, 
  unswipe,
  formatLocation,
  ...
  const totalPages = Math.ceil(otherSwipeList.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const currentSwipes = otherSwipeList.slice(startIndex, startIndex + itemsPerPage);
  ...
  return (
    <>
      {currentSwipes.length > 0 ? (
                ...
                <div className="flex justify-between lg:w-290 lg:absolute">
                  ...
                  <div className="mt-7 mr-2 lg:mt-0 lg:mr-0 lg:order-3 justify-items-end">
                    ...
                    <div className="flex justify-between items-center w-24 mt-6">
                      ...
                      <button
                        onClick={() => {callMerryAPI?.(swipe.id,"like");
                        }}
        ...
        <p className="text-center text-gray-500 mt-4">No one has liked you yet.</p>
      )}
